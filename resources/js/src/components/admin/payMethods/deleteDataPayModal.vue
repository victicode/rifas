...
import { useMethodPayStore } from '@/services/store/methodPay.store';
  ...
  const dataPay = ref(props.dataPay)
  ...
  const deleteDataPay = () => {
    ...
    payMethodStore.deleteMethodData(dataPay.value.id)
    .then((response) => {
        setTimeout(() => {
          loading.value = false;
          showNotify('negative', 'Datos de pago borrado con exito')
          updateList()
        },1000)
    })
    .catch((response) => {
  ...
  watch(() => props.dataPay, (newValue) => {
    dataPay.value = newValue
  });
              ...
              <q-card-section class="q-pt-none q-px-sm ">
                <div class="px-2">
                  ...
                  <div class="row mt-1 ">
                    ...
                    <div class="col-md-12 col-12 md:pr-2 mb-1 md:mb-0 my-2 flex justify-between" v-for="(item, key) in dataPay.data" :key="key">
