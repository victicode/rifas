...
import { useRifaStore } from '@/services/store/rifas.store';
  ...
  const deleteRifa = () => {
    ...
    rifaStore.deleteRifa(rifa.value.id)
    .then((response) => {
...
<template>
   <q-dialog v-model="dialog" class="deleteRifaDialog" persistent backdrop-filter="blur(8px)">
              ...
              <q-card-section class="q-pt-none q-px-sm ">
                <div class="px-2">
                   <div class="text-h6 text-center text-black text-bold">
                    ¿Seguro que deseas borrar la rifa "{{ rifa.title }}"?
                  </div>
...
@media (max-width: 768px){
  ...
  .deleteRifaDialog .q-dialog__inner--minimized{
    padding: 24px 1rem;
  }
  .dialog_documentCard {
    margin-left: 0%;
